...
const mongoDbAbsolutePath = "D:\\mongodb\\bin\\mongodump.exe";

const dbOptions = {
  // user: require("./config/keys").mongoUsername,
  // pass: require("./config/keys").mongoPassword,
  uri: require("./config/keys").backupMongoURI,
  host: "localhost",
  port: 27017,
  database: "kancelaria-prawna",
  autoBackup: true,
...
exports.dbAutoBackUp = async () => {
  ...
  if (dbOptions.autoBackup == true) {
    ...
    const start = mongoDbAbsolutePath ? mongoDbAbsolutePath : "mongodump.exe";
    ...
    const response = await exec(cmd, (error, stdout, stderr) => {
      if (this.empty(error)) {
        // check for remove old backup after keeping # of days given in configuration.
        if (dbOptions.removeOldBackup == true) {
